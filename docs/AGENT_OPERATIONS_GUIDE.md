...
| Git 브랜치/커밋/PR 발행 | `code-reviewer` | `branch-pr-workflow`, `commit-push-pr` |
| 사용자 대시보드 UI 구조 정의 (#28) | `UI Designer`, `Product Owner` | `.github/agents/`, `docs/AGENT_CAPABILITY_CATALOG.md` |
| #28 프론트엔드 구현 연결 | `Expert React Frontend Engineer` | `frontend/src/app/components/*`, `frontend/src/hooks/useMeeting.ts`, `frontend/src/lib/api.ts` |
| #28 백엔드 계약 확인/샘플 정리 | `API Architect` | `backend/app/runtime/meeting_reads.py`, `backend/app/types/storage.py` |
...
## Docker Template Policy (Parent Compose)

- 이 저장소는 API 전용 Docker 이미지(`docker/Dockerfile.api`)만 제공합니다.
- Compose 실행 파일은 저장소 루트에 두지 않고, 상위 리포지토리에서 사용할 템플릿(`docs/templates/docker-compose.parent.api.yml`)만 제공합니다.
- 상위 compose는 아래 환경변수를 기준으로 경로/포트를 제어합니다.
  - `MEETING_MOOD_TRACKER_SUBMODULE_PATH` (기본 `./MeetingMoodTracker`)
  - `FASTAPI_SERVER_PORT` (기본 `8000`)
- LLM 설정은 상위 compose의 `env_file: <submodule>/dev.env`를 기본으로 사용합니다.
...
- 매핑 표준:
  ...
  - 기능별 이슈 본문 규칙이 필요하면 `feature_list.json`의 `issue_rule` 필드를 사용합니다.
  - `issue_rule` 권장 키:
    - `objective`
    - `in_scope`
    - `out_of_scope`
    ...
    - `verification`
    - `done_criteria` (또는 `acceptance_criteria`)
  - 위 필드를 채우면 `scripts/sync_feature_issues.py`가 신규 Issue 생성 시 섹션을 자동 렌더링합니다.

## Issue #28 Sub-Agent Workflow

- 목적: `회의 감정 타임라인 / 회의 분위기 요약 / 에이전트 분석 보고서` UI 정의와 구현 연결을 역할별로 분리합니다.
- 기본 순서:
  1. `Product Owner`가 #28 범위, acceptance criteria, 보류 항목을 고정
  2. `UI Designer`가 IA, 화면 책임, 상태 UX, topic aggregate 표현 방식을 정의
  3. `API Architect`가 project-aware 조회 API와 응답 shape를 기준 계약으로 정리
  4. `Expert React Frontend Engineer`가 기존 컴포넌트와 구현 순서로 연결
- 역할 매핑:
  - FrontEnd 개발 에이전트 → `Expert React Frontend Engineer`
  - BackEnd 개발 에이전트 → `API Architect`
  - UI Designer 에이전트 → `UI Designer`
  - Product Owner 에이전트 → `Product Owner`
- 운영 메모:
  - #28은 사용자 요청에 따라 독립 이슈로 진행하되, `feature_list.json`의 활성 feature(#31)와 커밋/문서 범위를 혼동하지 않습니다.
  - 현재 백엔드 조회 API는 이미 project-aware 경로를 제공하므로, #28의 주된 실행 대상은 프론트 구조 정렬과 UI 명세입니다.
